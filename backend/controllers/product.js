...
exports.createProduct=(req,res)=>{
   ...
   form.parse(req,(err,fields,files)=>{
       ...
       if(!name||!description||!price||!category||!quantity||!shipping) return res.status(400)
       .json({error:'All fields are required'})
       let product=new Product(fields)

       if(files.photo){
           if(files.photo.size>1000000) return res.status(400)
...
exports.removeProduct=(req,res)=>{
    ...
    product.remove((err,delProduct)=>{
        if(err) return res.status(400).json({error:'Cannot delete the product'}) 
...
//most popular and new products display
// by sell-most popular=/products?sortBy=sold&order=desc&limit=4
//new arrival=/products?sortBy=createdAt&order=desc&limit=4
//if noparams then all products to be displayed
...
//{$ne:req.product}} -- not including product with passed id
...
exports.listBySearch = (req, res) => {
    ...
    exports.decreaseQuantity=(req,res,next)=>{
        ...
        Product.bulkWrite(bulkOps,{},(err,products)=>{
            if(err) return res.status(400).json({error:'Could not update product'})
            next()
